...
function ListOfMatches() {
  ...
  return (
    <section className={`relative flex overflow-hidden border-b border-grey-300 `}>
      <div className='w-full '>
        ...
        >
          {cargando ? (
            ...
            matchs && matchs.map((element) => <Match match={element} key={Math.random()} />)
          )}
        </div>
      </div>
      {
        <div className=''>
          <button
            ...
            className='absolute right-0 top-0 h-full w-12 bg-slate-500 opacity-50'
          >
            {' '}
            {'>'}
          </button>
          {position > 0 && (
            <button
              onClick={() => setPosition(position - 350)}
              className='absolute left-0 top-0 h-full w-12 bg-slate-500 opacity-50'
            >
              {' '}
              {'<'}
            </button>
