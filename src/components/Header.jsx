...
function Header() {
  const { changeModal, modal, favTeam, setAnimarCerrar } =
    useContext(FavTeamContext);
  const { cargando } = useTeams();

  return (
    <header className="border-grey-700 flex w-screen items-center justify-center border-b-2 bg-hexblack px-4 py-2 sm:px-12">
      <nav className="flex w-full items-center justify-between gap-4 md:container">
        ...
        <div className="flex gap-5">
          ...
          {cargando ? (
            <div className="relative flex h-12 w-12 cursor-pointer flex-col items-center justify-center rounded-full border-2 border-grey-500 px-5 py-2 text-white sm:h-16 sm:w-16">
              <p className="text-center font-spiegel text-sm leading-tight text-white">
                Loading
              </p>
            </div>
          ) : favTeam.name ? (
            <Link href={`/teams/${favTeam.name}`}>
              <div className=" relative flex h-12 w-12 cursor-pointer flex-col items-center justify-center rounded-full border-2 border-grey-500 text-white shadow-2xl sm:h-16 sm:w-16">
                <img
                  src={favTeam?.image}
                  ...
                  height={50}
                />
                <div className="absolute -right-2 -top-1 sm:-right-1.5 sm:-top-0">
                  <svg
