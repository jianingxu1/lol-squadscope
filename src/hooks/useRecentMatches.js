      ...
      if (favTeam?.name) {
        try {
          ...
          const res = await fetch(
            `https://esports-api.lolesports.com/persisted/gw/getSchedule?hl=en-US&leagueId=${filterT.id}`,
            {
              headers: {
                'x-api-key': '0TvQnueqKa5mxJntVWt0w4LpLfEkrV1Ta8rQBb9Z',
              },
          ...
          const filterGames = tournament.filter(
            (element) =>
          ...
          const filterWorlds = games.filter(
            (element) =>
              element.match.teams[0].name === `${favTeam.name}` ||
              element.match.teams[1].name === `${favTeam.name}`,
          );
